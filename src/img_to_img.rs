...
use crate::{
    ...
    utils::{
        ...
        utils::{check_and_create_directory, get_character_line_list_based_on_luma},
    },
};
...
pub fn image_to_image(
    ...
    if is_color{
        ...
        ascii_img = grayscale_to_ascii_img(&img, num_cols, character_type, is_white_bg);
    } else {
        let gray_img = img.grayscale();
        ascii_img = grayscale_to_ascii_img(&gray_img, num_cols, character_type, is_white_bg);
    }
    ...
    match check_and_create_directory(output_directory) {
        Ok(_) => {
            if let Some(filename) = filename {
                ...
                let _ = ascii_img.save(path);
                if output_directory.is_some(){
                    return Ok(format!("Image saved to path: {} as {}.jpg", output_directory.unwrap(), filename));
                }else{
                    return Ok(format!("Image saved to current directory as {}.jpg", filename));
                }
            }
...
fn grayscale_to_ascii_img(
    ...
    let mut out_image: ImageBuffer<Luma<u8>, Vec<u8>> = ImageBuffer::from_pixel(output_image_width, output_image_height, Luma([background_code]));
